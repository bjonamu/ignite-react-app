...
module.exports = {
  ...
  // ensure we're in a supported directory
    if (!isIrAppDirectory(process.cwd())) {
      print.error('The `ir-app add` command must be run in an ignite-compatible directory.')
...
Examples:
  ...
  // we don't (yet) support scoped npm packages, so bail
    if (name.startsWith('@')) {
      print.error(`Error: Ignite react App CLI doesn't support scoped npm packages yet, sorry about that. Try it without the prefix.`)
